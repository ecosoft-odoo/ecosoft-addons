...
class CommonBaseApi(models.AbstractModel):
    ...
    def get_login(self, api_data, ssl_context):
        ...
        if api_data.api_type == "rest_api":
            ...
            raise ValidationError(_("Unsupported API type: %s") % api_data.api_type)
        # Update token
    ...
    def _execute_xmlrpc_api(self, models, api_data, auth_token, payload=None):
        ...
        try:
            ...
            raise ValidationError(_("execute_context is not valid JSON: %s") % e) from e
    ...
    def _execute_rest_api(self, api_data, auth_token, payload=None, params=None):
        ...
        try:
            ...
            raise ValidationError(_("Connection error: %s") % e) from e
        except requests.HTTPError as e:
            message = _("HTTP error: %s") % e
    ...
    def action_call_api(self, code_api):
        ...
        try:
            ...
            if not api_data:
                raise ValidationError(_("API config not found for code: %s") % code_api)
            ...
            if api_data.auth_required:
                if api_data.auth_method == "static_token":
                    ...
                    if not auth_token:
                        raise ValidationError(
                            _("Static token is required but not set.")
                        )
                else:
                    ...
                    if not auth_token:
                        raise ValidationError(_("No valid authentication token."))
            ...
            return self._notify_user("success", _("API call successful."))
